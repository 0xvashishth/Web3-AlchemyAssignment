...
describe("TestModifyVariable", function () {
  it("should change x to 1337", async function () {
    ...
    const contract = await ModifyVariable.deploy(10, "xnx");
...
describe("StrModifyVariable", function () {
    it("should change str to Hello", async function () {
        const ModifyVariable = await ethers.getContractFactory("ModifyVariable");
        ...
        // wait for contract to be deployed and validated!
        await contract.deployed();
        await contract.modifyStr();
        const newStr = await contract.str();
        assert.equal(newStr.toString(), "Hello");
